...
class TactPlayer {
    constructor(tact, ipFinder, configLoader, sendEvent, listenEvent) {
        ...
        this.logs = []
        this.api = new api(this.tact.playEffect, this.configLoader.load(), this.sendEvent)
        this.initializeListeners()
    }

    initializeListeners() {
        this.listenEvent('find-ip', this.findIp.bind(this))
        this.listenEvent('refresh', this.refresh.bind(this))
    ...
    launch() {
        ...
        this.tact
            ...
            .onUpdateDevices((message) => {
                this.sendEvent('tact-device-update', message)
            })
            .onConnecting(() => {
            ...
            .onDisconnected(() => {
                this.hapticsConnectionState = false
                this.sendEvent('tact-device-disconnected') }
            )
            .connect()

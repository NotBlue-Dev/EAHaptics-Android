...
class Api {
    constructor(tactPlay, config, send) {
        this.sendEvent = send
        ...
        fetch(`http://${this.playerIp}:6721/session`).then(resp => resp.json()).then(json => {
            ...
            if (error.response) {
                if (error.response.status === 404) {
                    ...
                    this.sendEvent('log', 'in Menu/Loading or invalid IP')
                } else {
                    console.log(`${error.response.status}`)
                    this.sendEvent('log', `${error.response.status}`)
                }
            } else if (error.request) {
                console.log('Connection refused, game running ?');
                this.sendEvent('log', 'Connection refused, game running ?')
            } else {
                console.log('Error', `${error}`);
                this.sendEvent('log',`${error}`)
            }
